...
export function DashboardPage() {
  ...
  const { refresh } = useDashboard(30_000, selectedPiId)
  const { data, loading, error, lastRefresh } = useDashboardStore()
  ...
  return (
    <div className="h-full overflow-y-auto bg-corporate-surface">
      <PageHeader
        title={`Mejora Continua${data.pi_activo ? ` · ${data.pi_activo.nombre}` : ''}`}
        subtitle="Capacidad, backlog, alertas y reporte del PI"
        meta={data.pi_activo ? (
          <>
